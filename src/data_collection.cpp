...
Eigen::Matrix4f pose_to_mat(geometry_msgs::Pose cur_pose) {
  geometry_msgs::Quaternion quat = cur_pose.orientation;
	Eigen::Matrix3f quat_to_mat = Eigen::Quaternionf(quat.w, quat.x, quat.y, quat.z).toRotationMatrix();
	Eigen::Matrix4f mat_output = Eigen::Matrix4f::Identity();
          ...
          if(!first_seen) {
            ...
            try {
            	ros::Duration(2).sleep();
		          tf_l.waitForTransform("/base_link", camera_pose.header.frame_id, ros::Time(0), ros::Duration(5));
		          tf_l.transformPose("/base_link", camera_pose, base_link_pose);
		          tf_l.waitForTransform("/map", "/base_link", ros::Time(0), ros::Duration(5));
		          tf_l.transformPose("/map", base_link_pose, tag_map_pose);
				      mats_arr[0] = pose_to_mat (tag_map_pose.pose);
              Eigen::Matrix4f first_robot_pose = mats_arr[0] * (pose_to_mat(camera_pose.pose).inverse());
            ...
            {
              Eigen::Matrix4f prev = pose_to_mat(current_vis_msg.pose);
            ...
            else
            {
              ...
              mats_arr[cur_tag_id] = pose_to_mat(current_vis_msg.pose);;
              look_prev = true;
            }
          }
